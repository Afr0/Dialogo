...
export default class DialogoModel {
    ...
    */
    ...
    static async getUser() {
       let userModel = new DialogoModel(this.USER_CACHE_NAME);
    ...
    async getCache() {
       ...
       if((this.#cacheName !== DialogoModel.USER_CACHE_NAME)) {
    ...
    */
    ...
    #storeImgInIndexedDB(key, blob) {
       const request = indexedDB.open(DialogoModel.IMAGE_CACHE_NAME);
       ...
       request.onupgradeneeded = function(event) {
          ...
          db.createObjectStore(DialogoModel.IMAGE_STORE_NAME);
       };
   
       request.onsuccess = function(event) {
          let db = event.target.result;
          let transaction = db.transaction(DialogoModel.IMAGE_STORE_NAME, 'readwrite');
          let store = transaction.objectStore(DialogoModel.IMAGE_STORE_NAME);
          store.put(blob, key);
       };
    ...
    #retrieveImgFromIndexedDB(key) {
       return new Promise((resolve, reject) => {
          let request = indexedDB.open(DialogoModel.IMAGE_CACHE_NAME);
 
          request.onsuccess = function(event) {
            let db = event.target.result;
 
            let transaction = db.transaction(DialogoModel.IMAGE_STORE_NAME, 'readonly');
            let store = transaction.objectStore(DialogoModel.IMAGE_STORE_NAME);
